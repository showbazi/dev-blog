...
description: "Did you know it's possible to use Redis for your entire database?  caching, counters, sorted lists, and even queues. Learn how to deploy a Redis & Next.js app in a few short minutes."
banner: /blogs/first-blog/react-image.png
altText: "serverless redis with nextjs"
customID: fdcefcc7-320a-4d9e-ba53-347f89cv2vw4
---
...
1. <span>
     The feature list is server-rendered using `getServerSideProps`. We fetch
     all features and sort based on the score and title.
   </span>
1. <span>
     The initial feature list from the server populates the local cache of
     [SWR](swr.vercel.app). This ensures data remains up-to-date on the
     client-side.
   </span>
1. <span>
     When adding features or emails, a `POST` request is sent to an [API
     Route](https://nextjs.org/docs/api-routes/introduction) to update Redis.
   </span>
1. <span>
     Adding/upvoting features will mutate the client-side cache for [optimistic
     UI updates](https://swr.vercel.app/docs/mutation).
   </span>
1. <span>
     Adding/upvoting features will mutate the client-side cache for [optimistic
     UI updates](https://swr.vercel.app/docs/mutation).
